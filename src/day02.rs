...
fn parse_draw(draw: &str) -> Vec<(Cube, usize)> {
    draw.trim()
        .split(',')
        .map(|count| {
            let Some((count, color)) = count.trim().split_once(' ') else {
                unreachable!("data in bad format.")
            };
    ...
    input
        ...
        .map(|line| {
            let Some((id, data)) = line.split_once(':') else {
                unreachable!("data in bad format. check id of {line}")
            };
        ...
        assert_eq!(
            ...
            [
                ...
                Game {
                    id: 3,
                    draws: vec![
                        vec![(Green, 8), (Blue, 6), (Red, 20)],
                        vec![(Blue, 5), (Red, 4), (Green, 13)],
                        vec![(Green, 5), (Red, 1)]
                    ]
                },
                Game {
                    id: 4,
                    draws: vec![
                        vec![(Green, 1), (Red, 3), (Blue, 6)],
                        vec![(Green, 3), (Red, 6)],
                        vec![(Green, 3), (Blue, 15), (Red, 14)]
                    ]
                },
                Game {
                    id: 5,
                    draws: vec![
                        vec![(Red, 6), (Blue, 1), (Green, 3)],
                        vec![(Blue, 2), (Red, 1), (Green, 2)]
                    ]
                },
            ]
        );
